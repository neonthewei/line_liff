...
export function getLiffId(): string {
  ...
  if (path.includes("/profile")) {
    // 個人資料頁面使用專用的 LIFF ID
    return LIFF_ID_PROFILE || LIFF_ID_TRANSACTIONS || "";
  }
    ...
    if (typeof window !== "undefined" && !window.liff) {
      window.liff = {
        ...
        openWindow: (params: { url: string; external: boolean }) =>
          console.log(
            "LIFF openWindow called in dev mode with params:",
            params
          ),
      };
    }
...
export function login(redirectUri?: string) {
  if (typeof window === "undefined" || !window.liff) {
    console.error("LIFF not available, cannot login");
    return;
  }
  ...
  try {
    ...
    // 在 LINE 內部瀏覽器中，可能已經登入，無需額外登入
    if (isInLineInternalBrowser) {
      console.log("In LINE internal browser, login may not be needed");
      return;
    }
    ...
    // 執行登入
    console.log("Logging in with LIFF");
    if (redirectUri) {
...
// 在 LIFF 中導航到另一個頁面
export function navigateInLiff(
  path: string,
  ...
  const isListToTransaction =
    currentPath.includes("/transactions") &&
    targetPath.includes("/transaction") &&
    !targetPath.includes("/transactions");
  ...
  if (
    isInitialized &&
    !BYPASS_LIFF &&
    (liff.isInClient() || isInLineInternalBrowser)
  ) {
...
export function getLiffUrlParams() {
  ...
  if (isInitialized && !BYPASS_LIFF && typeof liff !== "undefined") {
    try {
      ...
      console.error(
        "Failed to get parameters from LIFF context:",
        contextError
      );
    }
  }
