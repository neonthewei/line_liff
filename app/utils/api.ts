 ...
 * @param userId 用戶ID
 * @param year 年份
 * @param month 月份 (1-12)，如果不提供則清除整年緩存
 */
export function clearTransactionCache(
  ...
  try {
    ...
    if (month) {
      ...
      console.log(`Clearing cache for key: ${summaryKey}`);
      sessionStorage.removeItem(summaryKey);

      // 清除月度統計緩存
      const statsKey = `category_stats_${userId}_${year}_${month}`;
      console.log(`Clearing cache for key: ${statsKey}`);
      sessionStorage.removeItem(statsKey);

      // 清除每日趨勢緩存
      const trendKey = `daily_trend_${userId}_${year}_${month}`;
      console.log(`Clearing cache for key: ${trendKey}`);
      sessionStorage.removeItem(trendKey);
    } else {
      // 清除年度交易緩存
      const cacheKey = `transactions_${userId}_${year}_yearly`;
      console.log(`Clearing cache for key: ${cacheKey}`);
      sessionStorage.removeItem(cacheKey);

      // 清除年度摘要緩存
      const summaryKey = `yearly_summary_${userId}_${year}`;
      console.log(`Clearing cache for key: ${summaryKey}`);
      sessionStorage.removeItem(summaryKey);

      // 清除年度統計緩存
      const statsKey = `category_stats_${userId}_${year}_yearly`;
      console.log(`Clearing cache for key: ${statsKey}`);
      sessionStorage.removeItem(statsKey);

      // 清除所有月份的緩存
      for (let m = 1; m <= 12; m++) {
        clearTransactionCache(userId, year, m);
      }
    }

    console.log(
      `Transaction cache cleared for user ${userId} (${year}${
        month ? `-${month}` : " yearly"
      })`
    );
  } catch (error) {
