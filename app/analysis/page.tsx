...
import { initConsoleCapture, addCustomLog } from "@/utils/debug";
import { Skeleton } from "@/components/shared/ui";

// Define a type to match ChartComponents props
...
// Dynamically import chart components with better loading strategy
const ChartComponents = dynamic<ChartComponentsProps>(
  () => import("@/components/analysis/ChartComponents"),
  {
    ssr: false, // Disable server-side rendering for charts
    loading: () => <SkeletonCharts />, // Show skeleton while loading
  }
);
...
export default function AnalysePage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      addCustomLog(
        `正在獲取 ${year}年${
          activeView === "month" ? `${month}月` : ""
        } 的交易數據`
      );
      console.log(
        `Fetching data for user ${userId}, year ${year}${
          activeView === "month" ? `, month ${month}` : ""
        }, view: ${activeView}`
      );
      ...
      if (cachedData) {
        try {
          ...
          if (Date.now() - timestamp < CACHE_EXPIRY) {
            ...
            console.log(
              `Using cached data: ${cachedTransactions.length} transactions`
            );
      ...
      if (activeView === "month") {
        ...
        console.log(
          `Fetched ${transactionsData.length} transactions from API for month ${month}`
        );
        ...
        // 獲取年度數據 - 並行請求
        const [transactionsResult, summaryResult] = await Promise.all([
          fetchTransactionsByUser(userId, year),
        ...
        summaryData = summaryResult;
        console.log(
          `Fetched ${transactionsData.length} transactions from API for year ${year}`
        );
    ...
    const lineData = Object.keys(timeGroups)
      ...
      .map((timeLabel) => {
        ...
        const totalForActiveTab =
          activeTab === "expense" ? summary.totalExpense : summary.totalIncome;

        // 避免除以零
        const percentage =
          totalForActiveTab > 0 ? (amount / totalForActiveTab) * 100 : 0;
