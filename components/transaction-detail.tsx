...
import { ChevronRight, Trash2, Check, ChevronDown, ChevronUp, ChevronLeft, Plus, Edit, X } from "lucide-react"
...
export default function TransactionDetail() {
  ...
  useEffect(() => {
    ...
    async function initialize() {
      ...
      const transactionId = new URLSearchParams(window.location.search).get("id") || "1"
  ...
  const renderCalendar = () => {
    ...
    for (let i = 1; i <= daysInMonth; i++) {
      ...
      const isSelected = transaction?.date === `${year}年${(month + 1).toString().padStart(2, '0')}月${i.toString().padStart(2, '0')}日`
  ...
  if (isLoading) return <div className="flex justify-center items-center h-screen">載入中...</div>
      ...
      <div className="space-y-4 px-4">
          ...
          <div className="flex flex-col">
            <div className="flex items-center justify-between">
              ...
              <div 
                ...
                tabIndex={0}
                onKeyDown={(e) => e.key === "Enter" && handleToggleEditCategory()}
                aria-label="展開類型選單"
              >
            ...
            <div 
              className={`overflow-hidden transition-all duration-300 ease-in-out ${
                isEditingCategory ? 'max-h-96 opacity-100 mt-4' : 'max-h-0 opacity-0'
              }`}
            >
              {!isAddingCategory ? (
                <div className="space-y-3">
                  <div className="grid grid-cols-3 gap-2">
                    ...
                    {isCategoryEditMode && (
                      <button
                        className="py-2 rounded-xl text-center bg-blue-100 text-blue-600 hover:bg-blue-200 flex items-center justify-center transition-all duration-300 ease-in-out animate-scaleIn"
                  ...
                  <button
                    className={`w-full py-2 rounded-xl flex items-center justify-center transition-all duration-300 ease-in-out transform ${
                      isCategoryEditMode 
                        ? 'bg-blue-100 text-blue-600 hover:bg-blue-200 scale-100' 
                        : 'bg-gray-100 text-gray-600 hover:bg-gray-200 scale-100'
                    }`}
        ...
        <div className="bg-white rounded-2xl p-4 shadow-sm space-y-4">
            ...
            <div className="flex items-center">
              {isEditingAmount ? (
                <input
                  ...
                  className="w-24 px-2 py-1 border border-gray-300 rounded-lg text-right focus:outline-none"
                  autoFocus
                />
              ) : (
                <div 
          ...
          <div className="flex flex-col">
            ...
            <div 
              className={`overflow-hidden transition-all duration-300 ease-in-out ${
                isEditingDate ? 'max-h-96 opacity-100 mt-2' : 'max-h-0 opacity-0'
              }`}
            >
          ...
          {/* 交易類型 */}
          <div className="flex items-center justify-between">
            <span className="text-gray-600">屬性</span>
            <div className="flex gap-2">
              <button
                className={`px-3 py-1 rounded-xl ${
                  transaction.type === "expense" ? "bg-[#22c55e] text-white" : "bg-gray-200 text-gray-600"
                }`}
              ...
              <button
                className={`px-3 py-1 rounded-xl ${
                  transaction.type === "income" ? "bg-[#22c55e] text-white" : "bg-gray-200 text-gray-600"
                }`}
            ...
            <div className="flex items-center">
              {isEditingNote ? (
                <input
                  ...
                  className="w-40 px-2 py-1 border border-gray-300 rounded-lg focus:outline-none"
                  autoFocus
                />
              ) : (
                <div 
        ...
        <div className="space-y-4 mt-8">
          ...
          <button
            ...
            className="w-full py-3 rounded-2xl bg-[#f1f5f9] text-gray-900 flex items-center justify-center"
          >
